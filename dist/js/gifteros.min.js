...
// Toggle iPad's Search Bar
function ipadSearchBar() {
    $('#search-bar').show();
    $('.main-nav>.navbar-brand').addClass('hide');
    $('.left-navbar-links').addClass('hide');
}
...
function ratingFilterBtns() {
    ...
    for (var i = 5; i > 0; i--) {
        output += `
            <div type="button" class="btn btn-outline-switch btn-sm filter-reviews filter-rating text-sm" data-id="${i}">
...
function deleteAlert(title, message, btnText, el, id) {
    Swal.fire({
        title: title,
        text: message,
        icon: 'question',
    ...
    var output = `
        ...
        <div class="flex-column justify-content-center w-100 text-center mt-2">
            ...
            <div class="d-block text-center position-relative">
                ...
                <ul class="list-inline popover-star-rating">
                ...
                <div class="d-flex align-items-center justify-content-center popover-review-count mb-1">
...
$(function() {
    ...
    // Toggle the active class on desktop-pills
    $('.desktop-pills').on('click', function() {
        $(this).addClass('active');
        $('.desktop-pills').not(this).removeClass('active');
    });

    // Toggle the active class on the nav-scroller links
    $('.nav-underline>.nav-link').on('click', function(e) {
        e.preventDefault();
        $(this).addClass('active');
        $('.nav-underline>.nav-link.active').not(this).removeClass('active');
    });
    ...
    // Toggle the add-recipient-btn
    $('.add-recipient-btn').on('click', function() {
        $(this).toggleClass('btn-outline-switch');
        $(this).toggleClass('bg-switch text-white');
        $(this).children(':first').toggleClass('bi-person-plus');
        $(this).children(':first').toggleClass('bi-person-check-fill');
        var user_id = $(this).data('user_id');
        var action = $(this).attr('id');
        if (action == 'add-recipient') {
            $(this).attr('id', 'remove-recipient');
        } else {
            $(this).attr('id', 'add-recipient');
        }
    });
    ...
    // Hide and display combo-total and combo-trigger
    $('#combo-details').on('show.bs.collapse', function() {
        $('#combo-trigger').addClass('d-none');
        $('#combo-total').removeClass('d-none');
        $('#combo-trigger').children(':first').toggleClass('bi-chevron-down');
        $('#combo-trigger').children(':first').toggleClass('bi-chevron-up');
    });

    $('#combo-details').on('hide.bs.collapse', function() {
        $('#combo-trigger').removeClass('d-none');
        $('#combo-total').addClass('d-none');
        $('#combo-trigger').children(':first').toggleClass('bi-chevron-down');
        $('#combo-trigger').children(':first').toggleClass('bi-chevron-up');
    });

    // Remove a combo-item when checkbox is uchecked
    $('.combo-item').on('click', function() {
        var gift_id = $(this).closest('li').data('id');
        $(`#combo-item${gift_id}`).closest('li').remove();
        comboItems();
    });

    // Loop through all combo-items and remove the combo-list 
    // if they're not found
    function comboItems() {
        var combo_items = $('.combo-item');
    ...
    // Remove (del) an item from the cart or any list
    $('.remove-item').on('click', function() {
        gift_id = $(this).data('id');
        var title = 'Remove from Cart?';
        var message = 'Are you sure that you want to remove this item from your cart?';
        var btnText = 'Yes, I\'m sure';
        var el = '#wishlist-item';
        deleteAlert(title, message, btnText, el, gift_id);
    });
    ...
    $('.quantity-input').on('change', function() {
        ...
        gift_price = parseFloat($(`#hidden-gift-price${gift_id}`).val());
        if (item_count > 1) {
            total_price = (item_count * gift_price).toFixed(2);
            ...
            comparison_item.innerHTML = `
                ...
                <div class="d-flex align-items-center">
                    ...
                    <div class="d-block">
                        ...
                        <h6 class="text-color-switch text-uppercase font-600">US$${gift_price}</h6>
                    </div>
                </div>
    ...
    // Not show the search bar on iPads and tablets when not in focus
    $(document).on('mouseup', function(e) {
        if (window.innerWidth >= 768 && window.innerWidth < 1024) {
            ipad_search_bar = $('#search-bar');
            if (!ipad_search_bar.is(e.target) && ipad_search_bar.has(e.target).length == 0) {
                $('#search-bar').hide();
                $('.main-nav>.navbar-brand').removeClass('hide');
                $('.left-navbar-links').removeClass('hide');
            }
        }
    });
